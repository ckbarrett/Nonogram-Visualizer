...
export function drawGrid() {
  ...
  const minDim = (gridWidthProportion < gridHeightProportion) ? window.innerWidth : window.innerHeight
  const numCells = (gridWidthProportion < gridHeightProportion) ? numGridCols : numGridRows
  const squareWidth = (minDim - (2 * margins) - (numCells + 1) * gridLineWidth) / numCells
  ...
  for(let i = 0; i <= numGridRows; i++){
    line(gridLineWidth / 2 + (squareWidth + gridLineWidth) * i, gridLineWidth / 2, gridLineWidth / 2 + (squareWidth + gridLineWidth) * i, gridHeight)
  }
  for(let i = 0; i <= numGridCols; i++){
    line(gridLineWidth / 2, gridLineWidth / 2+ (squareWidth + gridLineWidth) * i, gridWidth, gridLineWidth / 2 + (squareWidth + gridLineWidth) * i)
  }
}